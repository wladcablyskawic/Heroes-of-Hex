(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "army-"; // Prefix string to be prepended to the cart's name in the session storage
		...
		displayUserDetails: function() {
			if( this.$userDetails.length ) {
					var name = this.storage.getItem( "player-name" );
					var email = this.storage.getItem( "player-email" );
			...
			if( this.$formCart.length ) {
				...
				if( cartItems.length > 0 ) {
					...
					for( var j = 0; j < cartItems.length; ++j ) {
						...
						var cartPrice = this.currency + " " + cartItem.price*cartItem.qty;
		...
		_saveFormData: function( form ) {
			...
			$visibleSet.each(function() {
				...
				if( $set.is( "#fieldset-billing" ) ) {
					...
					self.storage.setItem( "player-name", name );
					self.storage.setItem( "player-email", email );
				} else {
